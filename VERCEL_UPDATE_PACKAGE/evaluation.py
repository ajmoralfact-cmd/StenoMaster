...
def tokenize_text(text: str, language: str = 'hindi') -> List[str]:
    """
    Tokenizes text into word tokens separated by whitespace, exactly matching
    the word segmentation used in TypingBaba.com, MeriTyping.com, and official
    government typing examinations (SSC, UPSSSC, High Court).
    Punctuation remains attached to its host word (e.g. 'है,' or 'देश।') to
    preserve word alignment and prevent phantom punctuation rows.
    """
    if not text:
        return []

    # Whitespace-based word splitting across all typing modes
...
def clean_word_for_similarity(w: str, language: str = 'hindi') -> str:
    ...
    and apply canonical Devanagari normalization for Hindi words.
    """
    if not w:
        return ""
    ...
    # Strip punctuation and non-word characters
    cleaned = re.sub(r'[^\w\u0900-\u097F]', '', w)
    if language == 'hindi':
        from hindi_converter import normalize_devanagari_canonical
        cleaned = normalize_devanagari_canonical(cleaned)
        # Chandrabindu / Anusvara equivalence in Hindi typing exams (e.g. जहाँ vs जहां)
        cleaned = cleaned.replace('\u0901', '\u0902')
    return cleaned
...
def analyze_hindi_word_difference(official_word: str, student_word: str) -> Tuple[str, str]:
    ...
    from hindi_converter import normalize_devanagari_canonical
    norm_off = normalize_devanagari_canonical(official_word)
    norm_stu = normalize_devanagari_canonical(student_word)

    # 1. Exact match under canonical Devanagari normalization
    if norm_off == norm_stu:
        return ("correct", "बिलकुल सही")

    clean_off = clean_word_for_similarity(official_word, 'hindi')
    clean_stu = clean_word_for_similarity(student_word, 'hindi')

    # 2. Check for pure punctuation error if base words are identical
    if clean_off == clean_stu:
        return ("punctuation", f"विराम चिह्न त्रुटि (अपेक्षित: '{official_word}', आपने टाइप किया: '{student_word}')")

    # 3. Check for Matra / Confusable pairs
    ...
    # 4. Check for Halant / Half-character error
    if ('्' in clean_off) != ('्' in clean_stu):
        sim = difflib.SequenceMatcher(None, clean_off, clean_stu).ratio()
        if sim >= 0.65:
            return ("character", f"आधे अक्षर / हलंत की अशुद्धि (अपेक्षित: '{official_word}')")

    # 5. Similarity check: if high similarity, it is a spelling / letter error
...
def _is_token_match(t1: str, t2: str, language: str = 'hindi') -> bool:
    ...
    if language == 'hindi':
        from hindi_converter import normalize_devanagari_canonical
        n1 = normalize_devanagari_canonical(t1)
        n2 = normalize_devanagari_canonical(t2)
        if n1 == n2:
            return True
    ...
    if language == 'hindi':
        if len(c1) >= 4 and len(c2) >= 4:
            return difflib.SequenceMatcher(None, c1, c2).ratio() >= 0.80
    elif language in ('krutidev', 'devlys'):
        n1 = t1.replace('¡', 'a').rstrip(',.A"\'')
        n2 = t2.replace('¡', 'a').rstrip(',.A"\'')
        if n1 == n2 or n1.replace('+', '') == n2.replace('+', ''):
            return True
        return difflib.SequenceMatcher(None, n1, n2).ratio() >= 0.80
    else:
        if c1.lower() == c2.lower():
            return True
        if len(c1) >= 4 and len(c2) >= 4:
            return difflib.SequenceMatcher(None, c1.lower(), c2.lower()).ratio() >= 0.80
...
def align_tokens_sequential(official_tokens: List[str], student_tokens: List[str], language: str = 'hindi') -> List[Dict[str, Any]]:
    ...
    def build_comparison_item(off_t: str, stu_t: str) -> Dict[str, Any]:
        ...
        if language == 'hindi':
            from hindi_converter import normalize_devanagari_canonical
            if normalize_devanagari_canonical(off_t) == normalize_devanagari_canonical(stu_t):
                return {
                    "status": "correct",
                    "official": off_t,
                    "student": stu_t,
            ...
            status = "correct" if err_type in ("none", "correct") else "wrong"
            return {
                "status": status,
                "official": off_t,
                "student": stu_t,
